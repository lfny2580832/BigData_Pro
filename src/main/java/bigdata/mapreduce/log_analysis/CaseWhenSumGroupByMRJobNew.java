...
import bigdata.io.TerminalTypeReportWritable;
import bigdata.lib.TerminalTypePartitioner;
...
public class CaseWhenSumGroupByMRJobNew extends Configured implements Tool {
    ...
    public static class CaseWhenSumGroupByReducer extends Reducer<Text, AdMetricWritable, NullWritable, TerminalTypeReportWritable>{
        //一个reduce task初始化一个terminalTypeReport
...
//            6.1 微信公众号
        ...
        protected void reduce(Text key, Iterable<AdMetricWritable> values, Context context) throws IOException, InterruptedException {
            //因一个reduce task处理的终端类型都一样，该语句给terminalTypeReport第一次赋值，之后就不用了
    ...
    public int run(String[] args) throws InterruptedException, IOException, ClassNotFoundException {
        ...
        job.setJarByClass(CaseWhenSumGroupByMRJobNew.class);
        ...
        job.setOutputValueClass(TerminalTypeReportWritable.class);//reduce输出value类型

        job.setNumReduceTasks(4);//硬编码，不灵活

        job.setPartitionerClass(TerminalTypePartitioner.class);//设置自定义的partitioner
